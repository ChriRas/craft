...
if [[ "$HOOK_BASH" == "/bin/bash" ]] && [[ "${hook_version%%.*}" -lt 5 ]]; then
  parse_fail=""
  for s in "$REPO_ROOT"/hooks/*.sh "$HELPER"; do
    /bin/bash -n "$s" 2>/dev/null || parse_fail="$parse_fail ${s#$REPO_ROOT/}"
  done
  [[ -z "$parse_fail" ]] && ok "hooks/*.sh and check-toolchain.sh parse under /bin/bash $hook_version" || bad "3.2 parse failure:$parse_fail"
...
[[ -z "$bash4_hits" ]] && ok "hooks/*.sh and check-toolchain.sh contain no construct newer than bash 3.2" \
